...
class WSDInstance:
    ...
    def __str__(self):
        ...
        '''
        return "%s\t%s\t%s\t%d" % (str(self.id), self.lemma, b' '.join(self.context), self.index)
...
def load_dataset(data_file_name, key_file_name):
    # Loading Data
    data_f = data_file_name
    key_f = key_file_name
...
if __name__ == '__main__':
    data_f = '../dataset/multilingual-all-words.en.xml'
    key_f = '../dataset/wordnet.en.key'
    dev_instances, test_instances = load_instances(data_f)
    dev_key, test_key = load_key(key_f)
